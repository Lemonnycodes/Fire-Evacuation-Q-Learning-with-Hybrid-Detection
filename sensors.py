...
class Sensors:
    ...
    def __init__(self, lower, upper, count):
        self.total_sensors = count
        self.lower_bound = lower
        self.upper_bound = upper
        self.sensors_location = set()
        self.configure_location()

    def configure_location(self):
        while len(self.sensors_location) < self.total_sensors:
            ...
            if (self.is_valid_location(x_loc, y_loc) and
                ...
                self.sensors_location.add((x_loc, y_loc))

    def is_valid_location(self, x_loc, y_loc):
        for existing_loc in self.sensors_location:
            if (
                    abs(existing_loc[0] - x_loc) < 3 and
                    abs(existing_loc[1] - y_loc) < 3
            ):
                return False  # Overlapping with an existing room
        return True

    def return_location(self):
        return self.sensors_location
